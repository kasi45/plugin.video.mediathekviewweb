...
class cMediathek:
    ...
    def getMediaData(self, params):
        ...
        # fix ARD wegen Filter
        _future=FUTURE
        if self.channel=='ARD' and self.query==None:
            _future=False

        m = MediathekViewWeb(PER_PAGE, _future)
        ...
        if not params['channel']=='None':
            ...
            for i in list:
                if i['channel']==params['channel']:
                    result.append(i)
        else: result=data["result"]["results"]
    ...
    def channelList(self):
        ...
        if isStaticChannelList:
            # statisch
            d = ["ARD","ZDF","SRF","NDR","SWR","BR","KiKA","SR","MDR","3Sat","WDR","ORF","PHOENIX","DW","ARTE.DE",
                 "ARTE.FR","HR","RBB","tagesschau24","Funk.net","rbtv","ZDF-tivi","ARD-alpha","ZDFinfo","ZDFneo",
                 "ONE","ARTE.EN","ARTE.ES","ARTE.PL","ARTE.IT","Radio Bremen TV","RBTV"]
            data = {'error': None,
                    'channels': ['ARD', 'ZDF', 'MDR', 'ARTE.DE', '3Sat', 'PHOENIX', 'NDR', 'SWR', 'SRF', 'ONE', 'BR',
                                 'SR', 'Radio Bremen TV', 'rbtv',"ORF", 'DW', 'HR', 'WDR', 'RBB',"ZDFinfo","ZDFneo",
                                 "ARD-alpha", 'ZDF-tivi', 'KiKA']}
        else:
            m = MediathekViewWeb()
            data = m.channels()
            if data["error"]:
    ...
    def getMedia(self,mediaFile=None, icon=None):
        media_files = ['3sat.png', 'ard-alpha.png', 'ard.png', 'arte.de.png', 'arte.fr.png', 'banner.png', 'br.png', 'downloads.png', 'dw.png', 'fanart.png',
                       'hr.png', 'icon.jpg', 'kika.png', 'lastsearch.png', 'mdr.png', 'ndr.png', 'next.png', 'one.png', 'orf.png', 'orf1.png', 'phoenix.png',
                       'radio bremen tv.png', 'rbb.png', 'rbtv.png', 'screenshot-01.jpg', 'search.png', 'sr.png', 'srf.png', 'srf.podcast.png', 'swr.png',
                       'tools.png', 'wdr.png', 'zdf-tivi.png', 'zdf.png', 'zdfinfo.png', 'zdfneo.png']
        ...
        if mediaFile in media_files:
            mediaFile = 'https://raw.githubusercontent.com/kasi45/plugin.video.mediathekviewweb/media/icons/%s' % mediaFile.replace(' ', '%20')
        elif control.exists(join(artPath, mediaFile)):
    ...
    def list_videos(self):
        ...
        for i in self.list:
            try:
                ...
                control.addItem(
                    syshandle,
                    '%s?action=play&url=%s&title=%s&channel=%s' % (sysaddon, url, title, i["channel"]),  # sysaddon(action="play", url=url, subtitle=i["url_subtitle"]),
                    li,
                    isFolder=False
                )
            except:
...
#TODO
        # ARD Patch
        if 'pd-videos.daserste.de/dach/' in url:
            url=url.replace('pd-videos.daserste.de/dach/', 'universal-vod.daserste.de/i/dach/')
            url =url.replace('/JOB_', '/,JOB_')
            url =url.replace('.mp4', ',.mp4.csmil/master.m3u8')

        kodiver = int(control.getKodiVersion())
        ...
        control.resolveUrl(syshandle, True, item)
